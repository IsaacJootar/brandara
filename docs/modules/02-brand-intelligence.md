...
- `/my-brand/voice` — Voice DNA setup and training
...
3. System sends samples to Claude via VoiceDnaService
4. Claude returns a voice profile JSON object
5. Stored in `brands.voice_dna`
...
## Database tables
- `brands` — all fields including voice_dna JSON
- `media_files` — logo storage

## Agency mode
Each client brand managed by an agency has its own completely
isolated Brand Kit, Brand Profile, and Voice DNA.
